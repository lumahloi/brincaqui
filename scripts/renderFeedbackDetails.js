...
function renderFeedbackDetails(item) {
  ...
  html += `
    <div class="p-3 card rounded" data-id="${item.aval_id}">
      ...
      <p class="small mt-2 mb-0" data-key="aval_description">${
        item.aval_description || ""
      }</p>
      <textarea class="form-control d-none mt-2 mb-2" data-edit="aval_description">${
        item.aval_description || ""
      }</textarea>
  `;
  ...
  fields.forEach(({ label, key, icon }) => {
    ...
    const category = key.replace("aval_grade_", "");
    ...
    <div class="d-flex flex-column gap-1 mt-2">
      <div class="d-flex flex-row gap-2 align-items-center">
        ...
        <div class="col" data-key="${key}">${grade} <span>${getClassificacaoLabel(
      grade
    )}</span></div>
      </div>

      <div class="star-rating animated-stars text-center w-100 d-none" data-edit="${key}">
        ${[5, 4, 3, 2, 1]
          .map(
            (n) => `
          <input type="radio" id="star${n}-${category}" name="rating-${category}" value="${n}">
          <label for="star${n}-${category}" class="bi bi-star-fill fs-3"></label>
        `
          )
          .join("")}
      </div>
  ...
  if (editable) {
    html += `
      ...
      <button class="btn btn-success mt-3 d-none save-edit-btn bg-gradient-1 border-0">Salvar alterações</button>
    `;
  }
