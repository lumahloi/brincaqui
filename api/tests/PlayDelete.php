...
class EmpDeleteTest extends TestCase
{
    ...
    {
        $loginData = [
            'email' => 'lumahcliente@gmail.com', // Use um usuário válido de teste (empresa)
            'password' => 'admin@123'
        ];
    ...
    public function testEmpDeleteSuccess()
    {
        ...
        $ch = curl_init($this->playUrl . $brinquedoId);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        $this->assertStringContainsString('Brinquedo deletado com sucesso', $decoded['message']);
    }

    public function testEmpDeleteWithoutSession()
    {
    ...
    public function testEmpDeleteWithoutToyId()
    {
        $sessionId = $this->loginAndGetSessionCookie();
        $this->assertNotEmpty($sessionId, 'Sessão não criada no login.');
